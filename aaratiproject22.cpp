...
private:
    static const int numFlights = 3; // Number of flights
    static const int seatsPerFlight = 5; // Seats per flight
    queue<string> passengerNames[numFlights]; // Queue for passenger names
    queue<int> seatNumbers[numFlights]; // Queue for seat numbers
    stack<string> cancelledNames[numFlights]; // Stack for cancelled passenger names
    stack<int> cancelledSeats[numFlights]; // Stack for cancelled seat numbers
    bool seatMap[numFlights][seatsPerFlight]; // Seat availability (tracks booked seats)

public:
    // Constructor to initialize the seatMap with false values (all seats are initially available)
    FlightReservationSystem() {
        for (int i = 0; i < numFlights; i++) {
            for (int j = 0; j < seatsPerFlight; j++) {
                seatMap[i][j] = false; // Initialize all seats as available
            }
        }
    }
    ...
    void bookTicket(int flightNo, string name, int seatNo) {
        ...
        flightNo--; // Adjust to zero-based index
        ...
        seatMap[flightNo][seatNo - 1] = true;  // Mark seat as booked
    ...
    void cancelTicket(int flightNo) {
        ...
        if (!passengerNames[flightNo].empty()) {
            ...
            cancelledNames[flightNo].push(name); // Push cancelled details onto stack
            cancelledSeats[flightNo].push(seat); // Push cancelled seat onto stack

            seatMap[flightNo][seat - 1] = false;  // Mark seat as free
